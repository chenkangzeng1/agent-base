...
/// Resolve session_id (priority: CLI arg > env var > auto-generate)
...
/// Clean up expired sessions (inactive for more than max_age_days)
pub fn cleanup_expired_sessions(base_dir: &Path, max_age_days: i64) -> Result<u32> {
    ...
    for entry in std::fs::read_dir(&sessions_dir)? {
        ...
        if let Some(last_active) = meta["last_active_at"].as_str() {
            if let Ok(last_active) = chrono::DateTime::parse_from_rfc3339(last_active) {
...
/// Resolve and create a session context
