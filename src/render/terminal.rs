...
use agent_base::{AgentResult, RuntimeEvent, PlanStepStatus};
...
impl TerminalRenderer {
    ...
    fn write_line(&mut self, s: &str) -> AgentResult<()> {
        ...
        self.writer.flush()
            .map_err(|e| agent_base::AgentError::internal(format!("flush error: {e}")))?;
        Ok(())
    }
    ...
    fn render(&mut self, event: RuntimeEvent) -> AgentResult<()> {
        ...
        match &event {
            ...
            RuntimeEvent::PlanUpdated { explanation, plan, .. } => {
                ...
                self.write_line(&format!(
                    "   {}",
                    self.dim(explanation.as_deref().unwrap_or(""))
                ))?;
                for item in plan {
