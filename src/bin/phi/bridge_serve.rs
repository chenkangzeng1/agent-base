//! ``phi serve --bridge`` — stdio NDJSON headless mode for SDK consumption.
//!
//! This is the legacy bridge protocol used by the Python and Node.js SDKs.
//! It uses a custom NDJSON protocol (not JSON-RPC 2.0 / MCP) with explicit
//! handshake, tool registration, session management, and cancel support.
//!
...
pub async fn run() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(std::path::PathBuf::from(&home).join(".phi-agent"))?;

    // LLM config is resolved once at server startup from environment.
    // Per-run config (model/api_key/base_url) sent by the SDK in the
    // Run message is NOT applied — the LLM client is fixed for the
